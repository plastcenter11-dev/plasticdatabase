...
export default function SalesInvoicePage() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editing) { await api.put(`/sales-invoices/${editing.id}`, payload); toast.success('تم تحديث الفاتورة'); }
  ...
  const openEdit = (inv) => {
    ...
    setForm({
      ...
      date: inv.date, discount: Number(inv.discount || 0), tax_rate: inv.tax_rate, paid: inv.paid,
  ...
  const handlePost = async (id) => {
    ...
    try { await api.post(`/sales-invoices/${id}/post`); toast.success('تم الترحيل'); loadData(); }
    catch (err) { toast.error(err.response?.data?.error || 'خطأ'); }
  };

  const handleDelete = async (id) => {
    if (!window.confirm('حذف هذه الفاتورة؟')) return;
    try { await api.delete(`/sales-invoices/${id}`); toast.success('تم الحذف'); setSelectedIds(s => s.filter(x => x !== id)); loadData(); }
    catch (err) { toast.error(err.response?.data?.error || 'خطأ'); }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try { await api.post('/sales-invoices/bulk-delete', { ids: selectedIds }); toast.success(`تم حذف ${selectedIds.length} فاتورة`); setSelectedIds([]); loadData(); }
    catch (err) { toast.error(err.response?.data?.error || 'خطأ'); }
  };

  const handlePrint = (inv) => {
    const cust = inv.Customer?.name || '-';
    const emp = inv.Employee?.name || '-';
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <table className="erp-table">
          <thead><tr><th><input type="checkbox" checked={selectedIds.length === filtered.length && filtered.length > 0} onChange={toggleSelectAll} /></th><th>رقم الفاتورة</th><th>التاريخ</th><th>العميل</th><th>المندوب</th><th>الإجمالي</th><th>المدفوع</th><th>المتبقي</th><th>الحالة</th><th>إجراءات</th></tr></thead>
          <tbody>
            ...
            {filtered.map(inv => (
              <tr key={inv.id} className={selectedIds.includes(inv.id) ? 'bg-blue-50' : ''}>
                ...
                <td className="text-sm text-gray-600">{inv.Employee?.name || '-'}</td>
          ...
          <form onSubmit={handleSave} className="space-y-4">
              ...
              <table className="erp-table">
                ...
                <tbody>{form.items.map((item, idx) => {
                  ...
                  return (<tr key={idx}>
                    ...
                    <td><input type="number" className="erp-input py-1 w-16" value={item.discount} onChange={e => updateFormItem(idx, 'discount', e.target.value)} /></td>
                    <td className="font-bold">{lineTotal.toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2})}</td>
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              <div className="flex flex-col justify-end">
                {(calcGross() - calcSubtotal() > 0 || Number(form.discount) > 0 || Number(form.tax_rate) > 0) && (
                  <div className="text-xs text-gray-500 mb-1 space-y-0.5">
                    {(calcGross() - calcSubtotal()) > 0 && <div className="text-orange-500">خصم الأصناف: - {(calcGross() - calcSubtotal()).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ج.م</div>}
                    {Number(form.discount) > 0 && <div className="text-red-500">خصم الفاتورة: - {calcDiscountAmount().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ج.م</div>}
